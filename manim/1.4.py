...
class RandomMatrix(Scene):
    ...
    def construct(self):
        ...
        size = (2, 2) 
        bounds = (1, 10)
        first2b2 = MobjectMatrix([
            [MathTex("M_{11}"), MathTex("M_{12}")],
            [MathTex("M_{21}"), MathTex("M_{22}")]], h_buff = 1.6)  
        second2b2 = MobjectMatrix([
            [MathTex("P(M_{11})"), MathTex("P(M_{12})")],
            [MathTex("P(M_{21})"), MathTex("P(M_{22})")]], h_buff = 2)
        ...
        m.append(MobjectMatrix([[MathTex("\\text{Gaussian} (\mu,\sigma^{2})") for y in range(size[1])] for x in range(size[0])], h_buff = 3.5))
        m.append(MobjectMatrix([[MathTex("\\text{Uniform} (a,b)") for y in range(size[1])] for x in range(size[0])], h_buff = 3.5))
        signs = [" + ", " - "]
        ...
        m.append(MobjectMatrix([
            [MathTex("M_{11}"), MathTex("M_{12}"),MathTex("M_{13}"), MathTex("..."), MathTex("M_{1P}")],
            [MathTex("M_{21}"), MathTex("M_{22}"), MathTex("M_{23}"), MathTex("..."), MathTex("M_{2P}")],
            [MathTex(), MathTex(),MathTex(), MathTex("\ddots"), MathTex()],
            [MathTex("M_{N1}"), MathTex("M_{N2}"),MathTex("M_{N3}"), MathTex("..."), MathTex("M_{NP}")]], h_buff = 1.6).shift(DOWN))
        m.append(MobjectMatrix([
            ...
            [MathTex("M_{N1}"), MathTex("M_{N2}"), MathTex("..."), MathTex("M_{NN}")]], h_buff = 1.6).shift(DOWN))
        size = (5, 5)
        bounds = (-10, 10)
        m.append(DecimalMatrix(np.round(np.random.uniform(*bounds,size=size), decimals=2), h_buff = 1.6).shift(DOWN))
        m.append(RadomMatrix(size=size, bounds=bounds, precision=2, vt=vt).shift(DOWN))
        ...
        self.play(Write(a))
        self.wait(1)
        self.play(Transform(a,m[0]))
        temp1=MobjectMatrix([[DecimalNumber(np.round(np.random.uniform(*bounds), decimals=2)) for y in range(2)] for x in range(2)], h_buff = 1.6)
        temp2=MobjectMatrix([[DecimalNumber(np.round(np.random.uniform(*bounds), decimals=2)) for y in range(2)] for x in range(2)], h_buff = 1.6)
        self.play(Transform(a,temp1))
        self.play(Transform(a,temp2))
        self.wait(1)
        self.play(Transform(a,second2b2))
        self.wait(1)
        ...
        b1 = Brace(m[6])
        b2 = Brace(m[7])
        b1tex, b2tex = b1.get_tex("N\\text{x}P"), b2.get_tex("N\\text{x}N") 
        self.play(Transform(a, m[6]),FadeIn(b1,b1tex))
        self.wait(1)
        self.play(Transform(a, m[7]), Transform(b1tex, b2tex),ReplacementTransform(b1,b2))
        self.wait(1)
        self.play(Wiggle(element_types[0],scale_value=2))
        self.play(Wiggle(element_types[1],scale_value=1.5))
        self.wait(1)
        ...
        self.play(
            Transform(a, m[8]),
            FadeOut(guas,guas_area,b2,b1tex),
        ...
        self.play(FadeOut(integer_arrow_copy,entry_equals_tex,integer_texN))
        ...
        for i in range(3):
            temp = DecimalMatrix(original_vals[i], h_buff = 1.6).shift(DOWN).scale(0.5)
            self.add(temp)
            self.wait(0.25)
        ...
        a = DecimalMatrix(original_vals[0], h_buff = 1.6).shift(DOWN).scale(0.5)
        ...
        for i in [5,13,19]:
            ...
            self.play(Create(curr_arrow),FadeOut(prev_arrow))
            prev_arrow = curr_arrow
        self.play(FadeOut(prev_arrow))
        iid_tex = Tex("iid = Independent and identically").next_to(axes3).shift(UP*0.5)
        iid_tex.add(Tex("distributed random variables").next_to(iid_tex,DOWN))
        self.play(Write(iid_tex))
        self.wait(1)
        self.play(FadeOut(iid_tex))
        self.wait(1)
        ...
        symetric_tex =  MathTex("p(M)","=","\prod_{i,j} M_{ij} = \prod_{i} M_{ii}\prod_{i>j} M_{ij}").scale(0.5).next_to(a,RIGHT)
        ...
        rect1_middle = SurroundingRectangle(entries[1][0]) 
        ...
        sysmetric_matrix = DecimalMatrix(np.tril(original_vals[0].T) + np.triu(original_vals[0], 1)).shift(DOWN).scale(0.5)
        ...
        # pick out half of the entries
        n2 = len(sysmetric_matrix.get_entries())
        n = int(n2**(1/2))
        entries = [sysmetric_matrix.get_entries()[i:i+n] for i in range(0,n2,n)]
        wiggle_list = []
        for i in range(n):
            for j in range(n):
                if i>=j:
                    wiggle_list.append(Wiggle(entries[j][i]))
        self.play(*wiggle_list)
        
        # N*(N+1)/2
        count_tex = MathTex("\\frac{N*(N+1)}{2}").next_to(axes3,RIGHT)
        ...
        x_list = MathTex("X"," = ","(","x_{1}",",","x_{2}",",","x_{3}",",","...",",","x_{N}",")").next_to(a.copy().shift(UP),UP).shift(UP)
        C_def = MathTex("C_{ij}(X) "," \equiv ","\\text{cov}(x_i,x_j)").next_to(x_list,DOWN)
        self.play(Write(x_list),FadeOut(axes3,uniform_areaN,uniformN,count_tex))
        self.wait(1)
        ...
        equivalent1 = MathTex("\\text{cov}(a,b) = \\text{cov}(b,a)").next_to(a,DOWN)
        ...
        H_def = VGroup(MathTex("H_{ij}"),Tex("\\textasciitilde"),VGroup(Tex("Encodes probablility to"),Tex("transition from state"),Tex("i $\\rightarrow$ j in a small time $\\Delta$t.")).arrange(DOWN)).arrange(RIGHT).move_to(x_list).shift(LEFT*2)
        ...
        self.wait(1)
        self.play(Wiggle(equivalent_Complex))
        self.wait(1)
        self.play(Wiggle(complex_symetric_matrix.get_entries()[2],1.5))
        self.wait(1)
        self.play(Wiggle(complex_symetric_matrix.get_entries()[10],1.5))
        self.wait(1)
        self.play(*[Wiggle(complex_symetric_matrix.get_entries()[i],1.5) for i in range(0,25,6)])
        self.wait(1)
