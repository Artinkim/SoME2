...
class RandomMatrix(Scene):
    ...
    def construct(self):
        ...
        m_group = VGroup()
        for i in range(3):
            temp = Matrix(np.round(np.random.uniform(*bounds,size=size), decimals=2), h_buff = 1.6)
            temp.scale(0.5)
            m_group.add(temp)
            self.add(temp)
            self.wait(1)
            self.play(m_group.submobjects[i].animate.scale(0.4).move_to(LEFT * 2*(1-i) + DOWN * 1.5))
            self.wait(1)
        
        # show ensemble as collection of 3 previously created matricies
        m = m_group.submobjects
        a, b = MathTex("\\{").scale(1.7), MathTex("\\}").scale(1.7)
        ensemble = Group(MathTex("ensemble: "), a, m[0].copy(), m[1].copy(), MathTex("..."), m[2].copy(), b).arrange(RIGHT)
        self.play(TransformMatchingShapes(m_group, ensemble))
        self.wait(5)
